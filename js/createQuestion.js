...
window.onload = function () {
  ...
  document.getElementById('create-form-btn').addEventListener('click', saveForm);
...
function showQuestionDialog(question = null, index = -1) {
  ...
  Swal.fire({
    ...
    html: `
      <div id="create-question-dialog">
        <div class="question-field">
          ...
          <input type="text" id="question-text" class="field-input" placeholder="Enter the question" value="${question?.text || ''}">
        </div>
        ...
        <div class="right-controls">
          <div class="required-toggle">
            ...
            <label class="switch">
              <input type="checkbox" id="required" ${question?.isRequired ? 'checked' : ''}>
          ...
          <div class="formatting-buttons">
            <button type="button" class="format-btn ${formatting.bold ? 'active' : ''}" data-format="bold"><b>B</b></button>
            <button type="button" class="format-btn ${formatting.italic ? 'active' : ''}" data-format="italic"><i>I</i></button>
            <button type="button" class="format-btn ${formatting.underline ? 'active' : ''}" data-format="underline"><u>U</u></button>
          </div>
        </div>
        ...
        <div class="type-marks-row">
          ...
          <div class="marks-field">
            ...
            <input type="number" id="mark" class="field-input" min="1" value="${question?.mark || 1}">
          </div>
        </div>

        <div id="text-section" style="display: none;">
          <label class="field-label">Correct Answer</label>
          <input type="text" id="correct-answer" class="field-input" placeholder="Enter the correct answer" value="${question?.correctAnswer || ''}">
        </div>
    ...
    didOpen: () => {
      ...
      document.getElementById('type').addEventListener('change', showCorrectSection);
    ...
    preConfirm: () => {
      ...
      if (type === 'text') {
        ...
        const selected = Array.from(document.querySelectorAll('input[name="correct"]:checked')).map(cb => cb.value);
        if (selected.length === 0) {
          Swal.showValidationMessage('Please select at least one correct answer');
          return false;
        }
        newQuestion.options = options;
...
function showCorrectSection() {
  ...
  if (type === 'text') {
    ...
    label.textContent = type === 'radio' ? 'Options (Select correct answer)' : 'Options (Select correct answers)';
    renderOptions();
  }
}
...
function updatePreview() {
  ...
  preview.innerHTML = `<span style="${style}">${text || 'Preview will appear here...'}</span>`;
}
...
function renderQuestions() {
  ...
  if (questions.length === 0) {
    ...
    const oldQuestions = container.querySelectorAll('.question-card, .form-results-card-title');
  ...
  questions.forEach((q, index) => {
    ...
    card.innerHTML = `
      <div class="question-number">Question ${index + 1} ${q.isRequired ? '<span style="color: red;">*</span>' : ''} (${q.mark} mark${q.mark > 1 ? 's' : ''})</div>
...
function saveForm(e) {
  ...
  const status = document.getElementById('active-toggle').checked ? 'active' : 'inactive';

  if (!title) {
    Swal.fire({ icon: 'error', title: 'Error', text: 'Please enter form title' });
    return;
  }

  if (questions.length === 0) {
    Swal.fire({ icon: 'error', title: 'Error', text: 'Please add at least one question' });
    return;
  }
  ...
  if (editMode && currentFormId) {
    ...
    forms[formIndex] = { ...forms[formIndex], title, description, status, questions };
  } else {
    forms.push({
